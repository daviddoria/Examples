...
const int N_FFT=8;
vector<cmpdbl> fft(vector<cmpdbl>) ;
//extern vector<cmpdbl> fft(vector<cmpdbl> Input) ;
...
int main()
{
	...
	s.MakeSinWave(SineWave);
	
	for (int i=0;i<N_FFT ; i++)
	{
		cout << endl << SineWave[i] << endl;
	}
	...
	{
		A.push_back(cmpdbl(SineWave[j],0));
	}

	for (int h=0; h<N_FFT ; h++)
	{
		cout<<"hello" ;
		cout << endl << A[h] << endl;
	}
