...
// 3 9 8
  ...
  //output eigen values
  vtkstd::cout << "Eigen values (decreasing order): " << vtkstd::endl;
  for ( int i=0; i<3; i++ )
    vtkstd::cout << v[i] << " ";

  //output eigenvectors
  vtkstd::cout << "Eigen vectors (same order as eigen values): " << vtkstd::endl;
  for ( int i=0; i<3; i++ )
  {
    //the columns of w are the eigenvectors
    vtkstd::cout << "Eigen vector " << i << " : " << w[0][i] << " " << w[1][i] << " " << w[2][i] << vtkstd::endl;
  }
