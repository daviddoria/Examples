...
int main (int argc, char *argv[])
{
  if (argc != 5)
    {
    cerr << "Usage: " << argv[0] << " DepthSortFlag ThetaResolution " <<
        " PhiResolution ScalarVisibilityFlag" << endl;
    return EXIT_FAILURE;
    }

  bool doDepthSort = static_cast<bool>(atoi(argv[1]));
  int theta = atoi(argv[2]);
  int phi = atoi(argv[3]);
  bool scalarVisibility = static_cast<bool>(atoi(argv[4]));
