...
int main(int argc, char *argv[])
{
  ...
  rwLeft->Create();
  //rwLeft->SetSize(200,200);
  
  vtkSmartPointer<vtkKWRenderWidget> rwRight = 
      vtkSmartPointer<vtkKWRenderWidget>::New();
  rwRight->SetParent(win->GetViewFrame());
  rwRight->Create();
  //rwRight->SetSize(200,200);
      
  app->Script("grid %s -column 0 -row 0 -sticky nw", rwLeft->GetWidgetName()); //"news" is North East West South
  app->Script("grid %s -column 1 -row 0 -sticky ne", rwRight->GetWidgetName());
